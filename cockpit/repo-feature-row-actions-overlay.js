...
(function () {
    ...
    function buildOverlay(row, trigger, clientX, clientY) {
        ...
        if (first) {
            try { first.focus({ preventScroll: true }); } catch (e) { /* optional focus */ }
        }
    }

    function isConverted(cell) {
        if (!cell || cell.dataset.contextOverlayReady !== "yes") return false;
    ...
    function applyRow(row) {
        ...
        if (!row._ghsyncContextMenuBound) {
            ...
            row._ghsyncContextMenuBound = true;
        }
    }
    ...
    window.addEventListener("scroll", function (event) {
        if (openOverlay && openOverlay.contains(event.target)) return;
        closeOverlay();
    }, true);
    apply();
})();
